...
int main()
{
	printf("----------Testing the Smithy Card Through cardEffect.----------\n");
	...
	printf("-----Testing shuffle during smithy-----\n");

	int draw;
	for(draw = 0; draw < 5; draw++)
	{
		drawCard(playerOne, &copyG);
		discardCard(handPos, playerOne, &copyG, 0);
	}

	memcpy(&actualG, &copyG, sizeof(struct gameState));
	int returnValue = cardEffect(smithy, choice1, choice2, choice3, &actualG, handPos, &bonus);
	...
	//Second is to check it drew from the right pile and that the deck shuffled
	//Should be all 5 cards minus the amount draw, so 5-3 or 2
